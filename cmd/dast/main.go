...
import (
	...
	"distributed-scanner/internal/api"
	"distributed-scanner/internal/config"
	"distributed-scanner/internal/distributed"
	"distributed-scanner/internal/engine"
	"distributed-scanner/internal/model"
	"distributed-scanner/internal/service"
...
func runScan(args []string) {
	...
	nativeScanner := engine.NewNativePortScanner()
	nucleiExecutor := engine.NewNucleiCheckExecutor(cfg.Scan.PocDir)
	runner := engine.NewRunner(nativeScanner, nucleiExecutor, cfg.Scan.PocDir)

	scanOpts := engine.ScanOptions{
		Targets:      rawTargets,
		Profile:      *profileFlag,
		PortOverride: *portsFlag,
		Policy:       model.DefaultPolicy("cli-tenant"),
	...
	if *mdFlag != "" {
		if err := exporter.ExportMarkdown(result, *mdFlag); err != nil {
			fmt.Printf("⚠️ 导出 Markdown 报告失败: %v\n", err)
		} else {
...
func runSubmit(args []string) {
	...
	streamFlag := fs.String("stream", distributed.DefaultRawStream, "原始任务 Redis Stream")
	...
	streamClient := distributed.NewStreamClient(rdb)

	scanRunID := model.NewUUID()
	rawInput := distributed.RawTaskInput{
		ScanRunID:  scanRunID,
		RawTargets: targets,
		PortRange:  *portFlag,
		Profile:    *profileFlag,
	...
	env, err := distributed.NewEnvelope(idempotencyKey, "RawTaskInput", rawInput, 3)
	if err != nil {
...
func runWorker(args []string) {
	...
	rawStreamFlag := fs.String("raw-stream", distributed.DefaultRawStream, "原始任务 Redis Stream")
	targetStreamFlag := fs.String("target-stream", distributed.DefaultTargetStream, "目标资产 Redis Stream")
	groupFlag := fs.String("group", distributed.DefaultGroup, "Consumer Group 名称")
	_ = fs.Parse(args)
	...
	worker := distributed.NewWorkerRuntime(rdb, *rawStreamFlag, *targetStreamFlag, *groupFlag, runner)
...
func runUpdate(args []string) {
	...
	updater := service.NewResourceUpdater()
...
func printSummary(res *engine.ScanResult) {
	fmt.Println("\n========================================================================")
